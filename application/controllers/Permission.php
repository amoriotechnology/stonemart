...
$this->db->query($sql);
 ...
 redirect("Permission/add_role");
    ...
    {
        $date=date('d-m-Y');
        $query='insert into sec_userrole values(null,"'.$_POST['user_id'].'","'.$_POST['role_id'].'","'.$_SESSION['user_id'].'","'.$date.'")';
         $sql=$this->db->query($query);

         if($sql)
         {
             $this->session->set_flashdata('message','Added_successfully');
    ...
    {
        $query='DELETE FROM sec_userrole WHERE `sec_userrole`.`id` ='.$id;
       $sql=$this->db->query($query);
    ...
    public function insert_role()
    {
        ...
        if($num<1)
        {
            $sql = "insert into sec_role (type,uid)values ('".$_POST['rolename']."','".$_POST['uid']."')";
$this->db->query($sql);
